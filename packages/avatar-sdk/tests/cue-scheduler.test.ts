...
describe("CueScheduler", () => {
  ...
  it("queues behind a non-interruptible cue and promotes it after expiry", () => {
    ...
    subject.schedule(cue("motion", "nod", { priority: 100 }), 10);

    expect(subject.snapshot().queued).toHaveLength(1);
    const completed = subject.tick(101);
    expect(completed.active.gesture?.cue.name).toBe("nod");
  ...
  it("honors after_current_motion and emits motion lifecycle callbacks", () => {
    ...
    const wave = cue("motion", "wave", { duration_ms: 500 });
    subject.schedule(wave, 0);
    subject.schedule(
      cue("motion", "nod", { start_anchor: "after_current_motion" }),
      5,
    );

    expect(subject.snapshot().queued).toHaveLength(1);
    subject.notifyMotionEnded(wave.cue_id, 100);
    expect(subject.snapshot().active.gesture?.cue.name).toBe("nod");
  ...
  it("keeps the queue bounded", () => {
    ...
    subject.schedule(
      cue("motion", "wave", { interruptible: false, duration_ms: 1_000 }),
      0,
    );
    subject.schedule(cue("motion", "nod"), 1);
    subject.schedule(cue("motion", "wave"), 2);
    subject.schedule(cue("motion", "nod"), 3);
