...
- Require HTTPS for public remote origins. For trusted LAN trials, allow HTTP to literal RFC 1918 IPv4 addresses (10/8, 172.16/12, 192.168/16) and 127.0.0.1; SSH forwarding is optional. Apply the same address validation in the connection form and native persistence. The desktop connect-src permits HTTP/WebSocket transports for user-selected LAN addresses because CSP cannot express private IP ranges; application validation remains the endpoint boundary. Do not weaken loopback validation of native sidecar bootstrap. Connection changes tear down product surfaces, stop the local supervisor and reload all desktop windows; session IDs are namespaced by endpoint.
