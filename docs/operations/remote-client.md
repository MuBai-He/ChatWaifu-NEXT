...
打开 Web 页面或新版桌面客户端，首次会出现「连接 ChatWaifu」。选择「连接远程服务器」，填写服务器根地址（如 `https://api.example.com`）和服务器 `server.env` 中生成的管理令牌。验证通过后进入界面。桌面版从现有控制中心进入「连接 → 更改连接」；Web 版使用「连接设置」按钮。可以更换地址或恢复本地模式，服务器暂时离线时也能打开连接设置。

桌面版的「连接 → 管理对话」集中管理参与者与场景，不在桌宠输入框或控制按钮上叠加浮动标签。对话弹窗支持关闭按钮、取消、Esc 和点击遮罩退出；请求等待与失败不会锁住退出操作，关闭会取消本次请求并丢弃迟到响应。连接设置支持关闭和返回，打开时保留原页面与当前会话；仅提交新连接后重新加载客户端。
...
局域网试运行可直接填写 `http://192.168.1.103:18780` 这样的服务器地址，无需 SSH 转发。HTTP 允许私有 IPv4 地址（`10.*`、`172.16.*` 至 `172.31.*`、`192.168.*`）和 `127.0.0.1`；公网地址仍需 HTTPS。HTTP 仅用于可信局域网，访问令牌不经 TLS 加密。反向代理需监听服务器的局域网 IP，Runtime 仍保持回环监听，并将实际 Host/Origin 加入允许列表。macOS 如询问局域网访问权限，需要允许客户端访问。

SSH 隧道只在无法直接访问服务器时作为可选方式。它只转发 API 与 WebSocket 的 TCP 流量，不会自动转发 WebRTC 媒体。
...
2026-09-22 局域网桌宠验收：macOS 源码桌宠已使用私有 IPv4 HTTP 地址直接连接服务器，移除本机 SSH 转发后，会话、事件 WebSocket 与音频 WebSocket 正常建立。Web/桌面前端构建、ESLint、Rust fmt/clippy 和已有连接策略检查通过；本次未重新验收麦克风通话，也不代表安装包或公网语音已验收。
