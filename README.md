...
桌宠上方细线区域可拖动，点击人物会触发角色互动；右下角 `◇` 打开桌宠设置，`◉` 连接麦克风。
菜单栏的“宁”提供显示/隐藏角色、打开桌宠设置、切换鼠标穿透和退出。独立设置窗口包含桌宠、
声音、模型和数据四类设置，不再复用 Galgame 对话界面。窗口位置、尺寸、置顶、鼠标穿透与 HUD
状态保存在系统应用配置目录，不进入仓库；设置变更会实时同步到悬浮窗。设置窗口不会成为第二个
